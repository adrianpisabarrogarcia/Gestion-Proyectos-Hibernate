 ...
 * @author unknown
 */
public class SuministrosPorPiezaView extends JFrame {
    public SuministrosPorPiezaView() {
        initComponents();
        this.setTitle("Suministros por piezas a piezas");
        cargarCombo();
        cargarTabla();
    }
    ...
    private void cargarTabla() {
        ...
        ArrayList<ProveedoresEntity> proveedores = GestionController.getGestionFilterByPizasToProveedor(idProveedor);
        ArrayList<ProyectosEntity> proyectos = GestionController.getGestionFilterByPizasToProyecto(idProveedor);

        Object[][] data = new Object[proveedores.size()+proyectos.size()][ProveedoresEntity.getColumns().length+ProyectosEntity.getColumns().length];
        for (int i = 0; i < proveedores.size(); i++) {
            ProveedoresEntity proveedoresEntity = proveedores.get(i);
            ProyectosEntity proyectosEntity = proyectos.get(i);
            Object[] proveedoresArray = proveedoresEntity.toArray();
            Object[] proyectosArray = proyectosEntity.toArray();
            for (int j = 0; j < proveedoresArray.length; j++) {
                data[i][j] = proveedoresArray[j];
            }
            for (int j = 0; j < proyectosArray.length; j++) {
                data[i][j+proveedoresArray.length] = proyectosArray[j];
            }
        }


        String[] columns = new String[ProveedoresEntity.getColumns().length+ProyectosEntity.getColumns().length];
        for (int j = 0; j < ProveedoresEntity.getColumns().length; j++) {
            columns[j] = ProveedoresEntity.getColumns()[j];
        }
        ...
        labelOne.setText("Proveedores: "+proveedores.size());
        labelOne2.setText("Proyectos: "+proyectos.size());
    }

    private void cargarCombo() {
        ArrayList<PiezasEntity> piezas = ProveedoresController.getPiezas();
        for (PiezasEntity pieza : piezas) {
            comboInfo.addItem(""+pieza.getId());
        }
        if (piezas.size() > 0) {
    ...
    private void comboInfoItemStateChanged(ItemEvent e) {
        ...
        PiezasEntity pieza = PiezasController.getPieza(Integer.parseInt(id));
        if (pieza != null) {
            tpObjeto.setText(pieza.toString());
            cargarTabla();
        }
    }
